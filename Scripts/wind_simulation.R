...
rand_sim <- wind_sim(
  data_path = c("wind-data/20221011/00"),
  coords = lapply(seq(1, dim(start_points)[1]), function(x)
    as.matrix(start_points)[x, ]),
  nforecast = 48,
  nsim = 50,
  atm_level = "950mb",
  full = T,
  ...
  for (k in 1:50) {
    ...
    trajectories[[1]] <-
      bind_rows(
        trajectories[[1]],
        tibble(
          start_point =  rep(i, dim(start_end)[1] - 1),
          nsim = rep(k, dim(start_end)[1] - 1),
          nforecast = seq(1, dim(start_end)[1] - 1),
          dist = as.numeric(cumsum(
            st_distance(start_end[-1,],
                        start_end[-nrow(start_end),],
                        by_element = TRUE) / 1000
          )),
          azimuth = sapply(2:dim(start_end)[1], function(x)
...
png("Figures/distances_12h.png", width = 600)
bind_rows(
  trajectories[[1]] %>% filter(nforecast <= 12) %>% group_by(start_point, nsim) %>% summarise(dist = max(dist)) %>% mutate(model = "CA"),
  trajectories[[2]] %>% filter(nforecast <= 12) %>% group_by(start_point) %>% summarise(dist = max(dist)) %>% mutate(model = "HYSPLIT")
) %>%
  group_by(model) %>%
  mutate(median_dist = median(dist)) %>%
  ggplot(aes(x = dist)) + geom_density(aes(fill = model), alpha = .7) +
  geom_vline(aes(xintercept = median_dist, colour = model),
             linetype = "dashed",
             size = 1) +
  theme_bw() +
  ...
  scale_colour_manual(values = c("dark green", "darkgoldenrod"),
                      name = "Model") +
  xlab("Distance travelled over 12 hours (km)")
dev.off()

png("Figures/elevation_trajectories.png", width = 600)
bind_rows(lapply(hysplit_sim_traj, function(x)
  x %>%
    mutate(elevation = as.numeric(elevation),
           elev_m = as.numeric(elev_m)))) %>%
  ggplot(aes(x = nforecast, y = elev_m)) +
  geom_hline(yintercept = c(500),
             linetype = "dashed",
             colour = "dark red") +
  geom_line(aes(group = start_point), alpha = .1) +
  geom_point(data = bind_rows(lapply(hysplit_sim_traj, function(x)
    x %>%
      mutate(elevation = as.numeric(elevation),
             elev_m = as.numeric(elev_m)))) %>%
      ...
      slice(1L),
    colour = "grey50") +
  geom_smooth(colour = "dark blue") +
  theme_bw() +
  xlab("Hour of forecast") +
  ylab("Altitude (m)") +
  ggtitle("Altitudinal trajectories of HYSPLIT simulations")
dev.off()
...
for (k in 1:48) {
  ...
  for (i in 1:100) {
    ...
    t.azimuth <-
      t.test(x = deg2rad(range$azimuth),
             mu = deg2rad(hysplit$azimuth))

    if (t.dist$p.value > 0.05)
      dist_test[[k]][[i]] <-
      "same"
    else
      dist_test[[k]][[i]] <- "different"

    if (t.azimuth$p.value > 0.05)
      azimuth_test[[k]][[i]] <-
      "same"
    else
      azimuth_test[[k]][[i]] <- "different"
  }
}
...
for (k in 1:48) {
  ...
  for (i in 1:100) {
    ...
    azimuth_diff[[k]][[i]] <-
      deg2rad(abs(range$azimuth - hysplit$azimuth))
  }
}

all_diff <- bind_rows(lapply(1:48, function(x)
  tibble(
    start_point = as.factor(rep(1:100, times = 50)),
...
all_diff <- all_diff %>%
  mutate(
    x = sapply(all_diff$start_point, function(i)
      start_points[as.numeric(i), 1]),
    y = sapply(all_diff$start_point, function(i)
      start_points[as.numeric(i), 2])
  ) %>%
  left_join(bind_rows(lapply(hysplit_sim_traj,
                             function(x)
                               x %>%
                               mutate(elevation = as.numeric(elevation),
                                      elev_m = as.numeric(elev_m)))) %>%
              dplyr::select(-c(x, y)) %>%
              mutate(start_point = as.factor(start_point)))

m_dist <- mgcv::bam(dist ~ s(nforecast) + s(x, y) + s(elevation),
                    data = all_diff)

m_azimuth <-
  mgcv::bam(azimuth ~ s(nforecast) + s(x, y) + s(elevation),
            data = all_diff)
...
diff_new <- with(all_diff,
                 expand.grid(
                   nforecast = seq(1, max(nforecast), length = 48),
                   x = seq(min(x), max(x), length = 30),
                   y = seq(min(y), max(y), length = 30)
                 )) %>%
  mutate(elevation = 950)
diff_new$dist <- stats::predict(m_dist, diff_new, type = "response")
diff_new$azimuth <-
  stats::predict(m_azimuth, diff_new, type = "response")


err_dist <-
  stats::predict(m_dist, diff_new, se = T, type = "response")$se.fit
err_azimuth <-
  stats::predict(m_azimuth, diff_new, se = T, type = "response")$se.fit
...
diff_new %>%
  ...
  geom_ribbon(
    aes(ymin = lcl_dist, ymax = ucl_dist),
    fill = "darkgoldenrod",
    alpha = .5,
    colour = NA,
    data = diff_new %>%
      mutate(
        ucl_dist = dist + 1.96 * err_dist,
        lcl_dist = dist - 1.96 * err_dist
      ) %>%
      group_by(nforecast) %>%
      mutate(
        lcl_dist = min(lcl_dist),
...
diff_new %>%
  ...
  geom_ribbon(
    aes(ymin = rad2deg(lcl_azimuth), ymax = rad2deg(ucl_azimuth)),
    fill = "dark green",
    alpha = .5,
    colour = NA,
    data = diff_new %>%
      mutate(
        ucl_azimuth = azimuth + 1.96 * err_azimuth,
        lcl_azimuth = azimuth - 1.96 * err_azimuth
      ) %>%
      group_by(nforecast) %>%
      mutate(
        lcl_azimuth = min(lcl_azimuth),
...
plot_grid(
  ...
  border %>%
    ggplot() +
    geom_raster(
      aes(x = x, y = y, fill = rad2deg(z)),
      data = plot(sm(m_azimuth_viz, 2))$ggObj$data %>%
        drop_na()
    ) +
    geom_contour(
      aes(x = x, y = y, z = rad2deg(z)),
      data = plot(sm(m_azimuth_viz, 2))$ggObj$data %>%
        drop_na(),
      colour = "black"
    ) +
...
left_join(
  plot(sm(m_dist_viz, 2))$ggObj$data %>%
    drop_na() %>% dplyr::select(x, y, z) %>% rename(distance = z),
  plot(sm(m_azimuth_viz, 2))$ggObj$data %>%
    drop_na() %>% dplyr::select(x, y, z) %>% rename(azimuth = z)
) %>% mutate(azimuth = rad2deg(azimuth)) %>%
...
model_legend <-
  get_legend(
    bind_rows(
      trajectories[[1]] %>% mutate(model = "CA"),
      trajectories[[2]] %>% mutate(model = "HYSPLIT")
    ) %>%
      filter(start_point == 1) %>%
      ggplot(aes(
        x = model, fill = model, y = dist
      )) +
  ...
  print(plot_grid(
    plot_grid(
      ca_sim_traj[[i]] %>%
        ...
        geom_path(
          data = ca_sim_traj[[i]],
          aes(x = x, y = y, group = nsim),
          colour = "dark green",
          size = 1,
          alpha = .1
        ) +
        geom_path(
          data = hysplit_sim_traj[[i]],
          aes(x = x, y = y),
          size = 1,
